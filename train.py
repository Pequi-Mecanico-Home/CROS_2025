...
class Model(nn.Module):
    ...
    def plot_and_save_loss(self, output_dir, sequence):
        ...
        plt.ylabel("Loss")
        plt.legend()
        plt.title(f"Training and Validation Loss - Sequence {sequence}")
        plt.savefig(os.path.join(output_dir, f"loss_sequence_{sequence}.png"))
        plt.close()
    ...
    def auto_train(self, X_train, X_val, false_positive_val_data, steps=50000, max_negative_weight=1000,
        ...
        self.plot_and_save_loss(config["output_dir"], sequence=1)
        ...
        self.train_model(
                    ...
                    val_steps=val_steps, warmup_steps=steps//5,
                    hold_steps=steps//3, lr=lr, val_set_hrs=val_set_hrs)
        self.plot_and_save_loss(config["output_dir"], sequence=2)
        ...
        self.train_model(
                    ...
                    val_steps=val_steps, warmup_steps=steps//5,
                    hold_steps=steps//3, lr=lr, val_set_hrs=val_set_hrs)
        self.plot_and_save_loss(config["output_dir"], sequence=3)
    ...
    def train_model(self, X, max_steps, warmup_steps, hold_steps, X_val=None,
        ...
        for step_ndx, data in tqdm(enumerate(X, 0), total=max_steps, desc="Training"):
            ...
            if step_ndx in val_steps and step_ndx > 1 and X_val is not None:
                ...
                self.history["val_loss"].append(self.loss(val_predictions, y_val[..., None]).detach().cpu().numpy())
